...
Each team is assigned two sets of three nodes that are connected by a chain of Lightning Network channels:

`<spender> --- <router> --- <recipient>`

`<spender-cb> --- <router-cb> --- <recipient-cb>`
...
One of these three-node sets is configured with defaults and is easier to attack.
The other set includes "`-cb-`" in their aliases because they are integrated with
[circuitbreaker](https://github.com/lightningequipment/circuitbreaker), an LND
plugin specficially designed to mitigate channel jamming using rate limits.

### Crash Vulnerable Nodes

Each team will also be assigned nodes that are critically vulnerable to attack:

- A `gossip_vuln` node which runs LND version `18.2-beta` (released in July 2024)
    - https://morehouse.github.io/lightning/lnd-gossip-timestamp-filter-dos/

- A `onion_vuln` node which runs LND version `16.4-beta` (released in July 2023)
    - https://morehouse.github.io/lightning/lnd-onion-bomb/


Your goal is to execute these attacks by sending custom p2p messages to the target
node until it crashes.
...
> [!WARNING]
> Make sure you installed Warnet correctly -- at least version 1.1.20 is required.
> If you do not see the correct version, reinstall Warnet or ask for help.

```
$ warnet version
warnet version 1.1.20
```
...
The Warnet setup process will install
[`kubectl`](https://kubernetes.io/docs/reference/generated/kubectl/kubectl-commands)
into your local virtual environment, which provides lower-level Kubernetes API access.

This can be used to recover the seed phrase for your armada LND nodes, which
is saved to disk in the LND container:

```sh
(.venv) --> k exec armada-1-ln -- cat /tmp/genseed.json | jq
{
  ...
  "enciphered_seed": "APSVoj1sL6b1ptCiZR00Kkdi+5//83qT6SuRNqfZHEN6"
}
```
...
Since Lightning Networks require double the pods for the Lightning nodes,
these networks can be somewhat resource-intensive for local testing. For that
reason, the local testing network has been split in two.

- `regtest_jam`: For channel jamming attacks, only deploys the spender->router->recipient trio.
- `regtest_vuln`: For the Morehouse DoS attacks, only deploys the vulnerable versions of LND.

Deploy the local test network of your chouce by running the included script:

```
./scripts/deploy.sh regtest_jam
```
